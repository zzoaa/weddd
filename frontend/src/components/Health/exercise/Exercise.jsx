...
export default function Exercise() {
    ...
    async function getData() {
        try {
            const {data} = await api.get('/healthinfo/exercise/list/all')
            setData(data);
        }catch (e) {
            console.log(e)
            alert('불러오기 실패!')
    ...
    async function delData() {
        ...
        delArray.exerciseIds.push(del_id);
        try {
            await api.post('healthinfo/exercise/delete', delArray)
            setDelAlert({
                ...delAlert,
                active: false,
                idx: 0,
    ...
    async function addProcess() {
        ...
        try {
            ...
            const {data, status}= await api.post('healthinfo/exercise', addData)
            if(status === 200) {
                navigate(`${data.newId}`)
            }else {
                alert('알수 없는 오류 발생\n 관리자에게 문의주세요.')
    ...
    return (
        <section className='food_section'>
            ...
            <DefaultTable title={'추천운동 관리'} tableClass={'food_table'}>
                <thead>
                <tr>
                    <th>ID</th>
                    <th>등록된 아이콘</th>
