...
const controller = loadModule('inquiry', 'controller');
...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        ...
        let dir  = path.join(__dirname, '../../files/images/inquiry');//cst_id살려써야함..
        try {                // 디렉토리가 없는 경우 생성+
...
const fileFilter = function (req, file, cb) {
    const allowedExtensions = ['.jpg', '.JPG', '.jpeg', 'JPEG', '.png', '.PNG', '.webp', '.svg'];
    const fileExtension = path.extname(file.originalname);
...
router.put('/edit', controller.editConst); //1대1 문의 글 수정
router.delete('/del', controller.deleteConst);//1대1 문의 글 삭제하기
router.post('/list', controller.getConsts); // 1대1 문의 글들을 보여줄 경로
router.post('/details', controller.getConstDetails) //문의글 개별 불러오기
router.post('/write', controller.addConst); //1대1 문의 추가

router.get('/cat/list', controller.getCatList); //1대1 문의 카테고리 목록 불러오기
